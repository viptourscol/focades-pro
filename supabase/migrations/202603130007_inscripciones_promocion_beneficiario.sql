-- Promocion de aspirantes admitidos/legalizados a beneficiarios activos.

alter table public.inscripciones
  add column if not exists promovido_a_beneficiario boolean not null default false,
  add column if not exists promovido_at timestamptz,
  add column if not exists promovido_por_user_id uuid references auth.users(id) on delete set null,
  add column if not exists beneficiario_portal_id bigint;

create index if not exists idx_inscripciones_promovido_true
  on public.inscripciones (promovido_a_beneficiario)
  where promovido_a_beneficiario = true;

alter table public.portal_beneficiarios
  add column if not exists inscripcion_pk bigint,
  add column if not exists convocatoria_id bigint,
  add column if not exists convocatoria_nombre text;

create index if not exists idx_portal_beneficiarios_inscripcion_pk
  on public.portal_beneficiarios (inscripcion_pk);

-- Inserta beneficiario y deja trazabilidad en una sola operacion.
...
grant execute on function public.promover_inscripcion_a_beneficiario(bigint, integer, boolean) to authenticated;
